...
def solve_sudoku(G,a):
    ...
    return sud_helper(H,node,total_vertices,nc,given)



def sud_helper(H,node,total_vertices,nc,given):
    if node == total_vertices:
        print(nc)
        return True
    for color in range(9):
        print(nc)
        if node in given and nc[node] == color+1: 
            nc[node] = color+1
            if sud_helper(H,node+1,total_vertices,nc,given):
                return True  
        elif check_neighbors(H,node,nc,color+1):
            nc[node] = color+1
            if sud_helper(H,node+1,total_vertices,nc,given):
                return True    
    return False
